...
export class HomePage {
  ...
  getSuperficieInmuebles() {
    this.arrayFiltrado = this.propertiesService.getInmuebles().filter(u => (u.sqm >= this.min && u.sqm <= this.max ));
  }

  getPropertyPrice(price: string) {
    if(price == 'barato') {
      this.arrayFiltrado = this.propertiesService.getInmueblesBaratos();
      this.arrayFiltrado = this.arrayFiltrado.filter(u => (u.sqm >= this.min && u.sqm <= this.max ));
    } 
    if(price == 'lujo') {
      this.arrayFiltrado = this.propertiesService.getInmueblesDeLujo();
      this.arrayFiltrado = this.arrayFiltrado.filter(u => (u.sqm >= this.min && u.sqm <= this.max ));
